...
- 测试：`test/lang-strings.test.mjs` 9 项 focused（默认逐字节 deepEqual / en 切换 / 未知与继承键回落 / zh-en 表 key 形状一致防单边漂移 / 错误原文不翻译 / titlePrefix 组合 / createEventListener en 状态正文 / commands·actions·适配器段英文防 zh 副本回潮）；全量 `npm test` 1557 pass（1548 基线 + 9）。README 测试计数基线按 train close 惯例不动。
